...
RawPlayer.prototype._handle_onload = function() {
    var that = this
    this._set_status("initializing")
    var data = this.stream
    var pos = this.pos || 0
    var decoder = new Decoder()
    var remaining = this.stream.byteLength
    var ratio = null
    var filters = false
    var decode = function() {
      clearTimeout()
      ...
      if (remaining === 0) {
          ...
          var l = ( remaining < 1024) ? remaining : 1024 // 4096
...
RawPlayer.prototype.playback = function(url) {
  this._reset()
  this.url = url
  this.resumeReady = url
  var request = new XMLHttpRequest()
  request.open("get", this.url, true)
  request.responseType = "arraybuffer"
  var that = this
  request.onload = function(event) {
      that.stream = this.response
      that._handle_onload()
  }
...
/** @expose */
RawPlayer.prototype.stop = function(url) {
  if((this.url == url) || (url == undefined)){
