  ...
  constructor.prototype = {
    ...
    seek: function (index) {
      if (index < 0 || index > this.end) {
        throw new Error("Index out of bounds (bounds: [0, " + this.end + "], index: " + index + ").");
      }
      this.pos = index;
    },
  ...
  constructor.prototype = {
    readAll: function(callback,p) {
      ...
      this.stream.readAll(null, function (buffer) {
        ...
        this.currentStreamLength = this.reader.stream.length
    ...
    play: function(vid) {
      if(
      (!this.reader) || 
      (this.stream.url != vid.name)
      ){
        this.isPaused = false
        this.isPlaying = false
        this.stream.url = vid.name
        this.readAll(this.play.bind(this),vid);
        return;
      };
      ...
      var sps = avc.sps[0];
      var pps = avc.pps[0];
      var microStep = 31 // 1 // // Increase the processor workload iff Workers are disabled by reducing microStep
      ...
      setTimeout(function foo() {
        var avc = this.avc;
        video.getSampleNALUnits(this.pic).forEach(function (nal) {
          avc.decode(nal);
        });
        this.pic ++;
        if (this.pic < 3000 && !this.isPaused) {
          setTimeout(foo.bind(this), microStep); 
        };
      }.bind(this), 1);
      this.isPlaying = true
    },
...
var Broadway = (function broadway() {
  function constructor(div) {
    ...
    controls.setAttribute('style', "z-index: 100; position: absolute; bottom: 0px; background-color: rgba(0,0,0,0.25); height: 15px; width: 100%; text-align: left;");
    this.info = document.createElement('div');
    this.info.setAttribute('style', "font-size: 10px; color:#6e6e6e;");
